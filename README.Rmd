...
**Additional features**. When the output format is a Word document the function `ext_img()` enables the knitr chunk options `out.width` and/or `out.height` to be used to set the dimensions of a figure (R-generated or external image). The function `ioslides_level()` enables the use of the level one header # to separate slides in an ioslides presentation without producing no segue slides.
